...
> _Differentiation and integration formulas aren't particularly useful, as more comprehensive lists are provided in the exam paper._
...
- **Property of Absolute Values:** $||x| - |y|| \le |x - y|$
...
- **Result on Limits with Absolute Values:**
  $\displaystyle\lim_{x \to c} f(x) = 0 \iff \displaystyle\lim_{x \to c} |f(x)| = 0$
...
- **Standard Integrals:**
  ...
  - $\displaystyle\int \displaystyle\frac{1}{u\sqrt{u^2 - a^2}} du = (\displaystyle\frac{1}{a})\sec^{-1} |\displaystyle\frac{u}{a}| + C$
...
- **Volume of Solids of Revolution**:
  ...
  - **Shell Method (Rotation about x-axis)**: Volume =
    $2\pi \displaystyle\int_c^d y|f(y)| dy$ (where $f(y)$ defines
    the shape of the shell at $y$)

  - **Shell Method (Rotation about y-axis)**: Volume =
    $2\pi \displaystyle\int_a^b x|f(x)| dx$ (where $f(x)$ defines
...
- **Geometric Series**: A series of the form
  $\sum_{n=0}^{\infty} ar^n$. It converges if $|r| < 1$ and its sum is
  $a / (1 - r)$.
...
- **Absolute Convergence:** A series $\sum a_n$ converges absolutely
  if $\sum |a_n|$ converges. If a series converges absolutely, then it
  converges.

- **Root Test**: Let
  $L = \displaystyle\lim_{n \to \infty} \sqrt[n]{(|a_n|)}$
...
- **Ratio Test**: Let
  $L = \displaystyle\lim_{n \to \infty} |\displaystyle\frac{a_{n+1}}{a_n}|$
...
- **Radius of Convergence**: For a power series, the radius of
  ...
  that the series converges absolutely for $|x - a| < R$ and diverges
  for $|x - a| > R$.

- **Taylor Series**: If $f$ has a power series representation at
  $x = a$, that is $f(x) = \sum_{n=0}^{\infty} c_n(x - a)^n$,
  $|x - a| < R$ for some $R > 0$, then its coefficients are given by
...
- **Dot Product**:
  ...
  - $u \cdot v =$ $||u|| ||v||\cos(\theta)$, where $\theta$ is the
    angle between $u$ and $v$.

- **Projection of u onto v:**
  proj$_v u = (\displaystyle\frac{(u \cdot v)}{||v||^2})v$

- **Scalar Component of u in the direction of v:**
  comp$_v u = \displaystyle\frac{(u \cdot v)}{||v||}$
...
- **Cross Product**:
  ...
  - $||u \times v|| = ||u|| ||v||\sin(\theta)$, where $\theta$ is
