...
$\displaystyle\int_a^bf(x)dx=\lim_{n\to\infty}\left(\sum_{i=1}^n\left(\frac{b-a}{n}\right)f\left(a+k\left(\frac{b-a}{n}\right)\right) \right)=F(b)-F(a)$ given that $F$ is an anti-derivative of $f$. Alternatively, $f(x)=\displaystyle\frac{d}{dx}\int_x^a f(t)dt$.
...
For the series $\sum_{n=1}^\infty \vert a_n\vert$, let $\displaystyle L = \lim_{n \to \infty} \left| \frac{a_{n+1}}{a_n} \right|$:
...
- Apply the ratio test: $\displaystyle \left| \frac{u_{n+1}}{u_n} \right| = \lim_{n \to \infty} \left| \frac{c_{n+1}}{c_n} \right| \left| x - a\right|^2 = L \left| x - a\right|^2$.
- Converges if $L \left| x - a\right|^2 < 1 \iff \left| x - a\right| < 1/\sqrt{L}$.
- Diverges if $\left| x - a\right| > \frac{1}{\sqrt{L}}$. So, $R = 1/\sqrt{L}$.
...
1. $\displaystyle\frac{1}{1 - x} = \sum_{n=0}^\infty x^n, \ |x| < 1$
...
6. $\ln(1 - x) = -\sum_{n=0}^\infty \displaystyle\frac{x^{n+1}}{n+1}, \ |x| < 1$

---

## 3D Vector Geometry

- **Distance** from $(x_0,y_0,z_0)$ to a plane $ax+by+cz+d=0$ is $\displaystyle\left|\frac{ax_0+by_0+cz_0+d}{\sqrt{a^2+b^2+c^2}}\right|$.
