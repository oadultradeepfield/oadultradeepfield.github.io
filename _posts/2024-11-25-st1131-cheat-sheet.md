...
> _Concepts I Often Forget or Get Mixed Up_
...
- **Margin of Error (MOE)** represents the range within which we expect the true population parameter to fall, given a certain significance level. A smaller **MOE** suggests more **precise estimates**.

## Confidence Intervals (CI)

In the long run, **95% of intervals** will contain the true population parameter. **CI = Point Estimate $\pm$ Margin of Error.**

### Confidence Interval for Proportion

To find the CI ($D$) given a significance level ($x$):

1. Calculate $\hat{p}$ and check if $\hat{p}(1 - \hat{p}) \geq 5$.
2. Let $\alpha = 1 - x$.
3. $D=\hat{p}$ ± $Z_{\frac{1-\alpha}{2}} \times \sqrt{\displaystyle\frac{\hat{p}(1 - \hat{p})}{n}}$.

**Determine Sample Size ($n$) Before Study**:

1. Decide on the significance level ($x$) and the width of the CI ($D$).
2. Use the formula:
   $n \geq \left(\displaystyle\frac{Z_{\frac{1-\alpha}{2}}}{D}\right)^2 \times \hat{p}(1 - \hat{p})$, where $\hat{p} = 0.5$.

### Confidence Interval for Mean

t-distribution ($t_{\text{df}}$) approaches $N(0, 1)$ as degrees of freedom ($df$) increase.
To find the $D$ given a significance level ($x$):

1. **Assumptions**: The sample is random (not biased), and the data distribution is symmetric (or the sample size is large enough).
2. $D=\overline{X}$ ± $t_{n-1; \frac{1-\alpha}{2}} \times \displaystyle\frac{s}{\sqrt{n}}$.

**Determine Sample Size ($n$) Before Study**:

1. Decide on the significance level ($x$) and the width of the CI ($D$).
2. Use the formula:
   $n \geq \left(\displaystyle\frac{Z_{\frac{1-\alpha}{2}} \times s}{D}\right)^2$.
